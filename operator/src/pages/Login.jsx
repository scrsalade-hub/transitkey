...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    const stored = localStorage.getItem('transitkey_operator');
    if (stored) {
      const op = JSON.parse(stored);
      if (op.email === form.email && op.password === form.password) {
        login({ token: 'local_op_' + Date.now(), user: op });
        navigate('/');
        return;
      }
    }
    ...
    try {
      ...
      login(data); navigate('/');
    } catch (err) { setError(err.message || 'Invalid credentials. If you just registered, your account is saved locally.'); }
  };
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 bg-white px-4 py-6 md:py-12">
        <div className="w-full max-w-lg mx-auto bg-white border border-gray-300 p-4 md:p-8">
          <h2 className="text-lg md:text-xl font-semibold mb-1">Operator Login</h2>
          <p className="text-sm text-gray-600 mb-6">Access TransitKey command center.</p>
          {error && <div className="bg-red-50 text-red-600 text-sm p-3 mb-4 rounded">{error}</div>}
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1">Email Address</label>
              <input type="email" placeholder="name@gmail.com" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required className="w-full border border-gray-300 px-4 py-3 text-sm outline-none focus:border-blue-600" />
            </div>
            <div>
